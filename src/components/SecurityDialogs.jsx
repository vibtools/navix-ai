...
export function CredentialVaultDialog({ mode, error, onSessionOnly, onEncrypt, onUnlock, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-[75] bg-slate-950/50 backdrop-blur-xs flex items-center justify-center p-4" role="dialog" aria-modal="true" aria-labelledby="vault-title" aria-busy={busy}>
      <div className="w-full max-w-sm rounded-2xl border border-slate-200 bg-white p-4 shadow-2xl">
        <div className="flex items-start justify-between gap-3">
          ...
          {onClose && <button type="button" onClick={onClose} aria-label="Close credential vault dialog" className="rounded p-1 text-slate-400 hover:bg-slate-100"><X className="h-4 w-4" /></button>}
        </div>
        ...
        <div className="mt-4 flex flex-col gap-2">
          {encrypted ? (
            <button type="button" disabled={busy || passphrase.length < 10} onClick={() => run(onUnlock)} className="rounded-lg bg-blue-600 px-3 py-2 text-xs font-medium text-white disabled:opacity-40">Unlock</button>
          ) : (
            <>
              <button type="button" disabled={busy || passphrase.length < 10} onClick={() => run(onEncrypt)} className="rounded-lg bg-blue-600 px-3 py-2 text-xs font-medium text-white disabled:opacity-40">Encrypt and migrate</button>
              <button type="button" disabled={busy} onClick={() => run(onSessionOnly)} className="rounded-lg bg-slate-100 px-3 py-2 text-xs font-medium text-slate-700 disabled:opacity-40">Use session-only storage</button>
            </>
          )}
        </div>
