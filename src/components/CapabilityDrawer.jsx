...
export default function CapabilityDrawer({ item, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-[70] bg-slate-950/30" role="dialog" aria-modal="true" aria-labelledby="capability-drawer-title">
      <aside className="absolute right-0 top-0 flex h-full w-full max-w-[420px] flex-col border-l border-slate-200 bg-white shadow-2xl">
        <header className="flex items-center justify-between border-b border-slate-100 px-4 py-3"><div className="flex min-w-0 items-center gap-2"><FileSpreadsheet className="h-4 w-4 shrink-0 text-blue-600" /><h3 id="capability-drawer-title" className="truncate text-sm font-semibold text-slate-800">{artifact ? `Artifact · ${item.language}` : item.name}</h3></div><button type="button" onClick={onClose} aria-label="Close capability drawer" className="rounded-lg p-1.5 text-slate-400 hover:bg-slate-100"><X className="h-4 w-4" /></button></header>
        ...
        <footer className="flex gap-2 border-t border-slate-100 p-4">
          {artifact ? <button type="button" onClick={() => downloadBlob(item.content, 'text/plain;charset=utf-8', filename)} className="flex flex-1 items-center justify-center gap-1.5 rounded-lg bg-blue-600 px-3 py-2 text-xs font-medium text-white"><Download className="h-3.5 w-3.5" />Download</button> : <>
            <button type="button" onClick={() => downloadBlob(serializeCsv(item.rows || []), 'text/csv;charset=utf-8', `${filename}.csv`)} className="flex flex-1 items-center justify-center gap-1.5 rounded-lg bg-slate-100 px-3 py-2 text-xs font-medium text-slate-700"><Download className="h-3.5 w-3.5" />CSV</button>
            <button type="button" onClick={() => downloadBlob(createXlsx(item.rows || []), 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet', `${filename}.xlsx`)} className="flex flex-1 items-center justify-center gap-1.5 rounded-lg bg-blue-600 px-3 py-2 text-xs font-medium text-white"><Download className="h-3.5 w-3.5" />Excel</button>
          </>}
        </footer>
      </aside>
    </div>
